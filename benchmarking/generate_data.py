...
def generate_generic_datetime_data(
    stay_ids: list[int],
    max_rows: int,
    start_time: datetime,
    timestep: timedelta,
...
def generate_icu_data(max_rows: int, missingness_threshold: tuple[int, int] = (0.1, 0.7), seed: int = None) -> pl.DataFrame:
    ...
    categorical_columns = {
        "surgery": ["Cardiac", "General", "Orthopedic", "Neurosurgery", "Vascular"],
    ...
    generated_df = generate_generic_datetime_data(
        stay_ids, max_rows, start_time, timestep, columns, missingness, categorical_columns
    )
    return generated_df
